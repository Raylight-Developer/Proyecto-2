...
/**
 * @brief Generates a vector of random BCRYPT_KEY_HANDLEs using the specified algorithm handle.
 *
 * This function generates a set of random symmetric keys within the range specified by `start` and `end`.
 * Each key is generated by filling an 8-byte array with random values.
 * If the key generation fails, an error message is logged, and the key is skipped.
 *
 * @param hAlgorithm The handle to the cryptographic algorithm provider.
 * @param start The starting index for key generation.
 * @param end The ending index for key generation.
 * @return A vector of BCRYPT_KEY_HANDLEs representing the generated random keys.
 */
...
/**
 * @brief Generates a vector of ordered BCRYPT_KEY_HANDLEs using the specified algorithm handle.
 *
 * This function generates symmetric keys in a deterministic manner by using the index to fill an 8-byte array.
 * The byte array is filled by shifting the index value, allowing the generated keys to be ordered.
 * If key generation fails, an error message is logged, and the key is skipped.
 *
 * @param hAlgorithm The handle to the cryptographic algorithm provider.
 * @param start The starting index for key generation.
 * @param end The ending index for key generation.
 * @return A vector of BCRYPT_KEY_HANDLEs representing the generated ordered keys.
 */
